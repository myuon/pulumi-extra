...
export const createCORSResource = (
  name: string,
  option: {
...
export const createLambdaMethod = (
  name: string,
  option: {
    ...
    restApi: aws.apigateway.RestApi;
    integration: Omit<
      aws.apigateway.IntegrationArgs,
      "httpMethod" | "integrationHttpMethod" | "resourceId" | "restApi" | "uri"
    >;
    handler: aws.lambda.Function;
    method?: Omit<
      aws.apigateway.MethodArgs,
      "authorization" | "httpMethod" | "resourceId" | "restApi"
    >;
  }
) => {
  ...
  const integration = new aws.apigateway.Integration(
    name,
    {
      ...
      resourceId: option.resource.id,
      restApi: option.restApi,
      uri: pulumi.interpolate`arn:aws:apigateway:${aws
        .getRegion()
        .then(val => val.name)}:lambda:path/2015-03-31/functions/${
        option.handler.arn
      }/invocations`,
      ...option.integration
    },
  ...
  new aws.lambda.Permission(
    name,
    {
      ...
      sourceArn: pulumi.interpolate`arn:aws:execute-api:${aws
        .getRegion()
        .then(val => val.name)}:${current.accountId}:${option.restApi.id}/*/${
        method.httpMethod
